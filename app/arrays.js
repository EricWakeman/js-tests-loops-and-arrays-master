...
function flightCost(destination, firstClass) {
    ...
    let result = flights.find(({ to }) => to == destination.toUpperCase())
    if (firstClass) {
...
function findById(id) {
    let fail = { error: "No user with that id." };
...
function bandMemberDetails(name) {
    ...
    for (let index = 0; index < theBand.members.length; index++) {
        const element = theBand.members[index];
        if (name == element.name) {

            return `${name} is in the band and plays the ${element.instrument}`
        }
